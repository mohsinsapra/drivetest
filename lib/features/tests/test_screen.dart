...
class _TestscreenState extends State<Testscreen> {
  ...
  /// Translate a question's tabs (title + body text), preserving any embedded
  /// <img> tags so the tab gallery keeps working. Tab bodies are HTML that the
  /// widget strips to plain text for display, so we translate the stripped
  /// text and re-append the original images. Each item is translated safely so
  /// one failure can't break the batch.
  Future<List<QuestionTab>> _translateTabs(List<QuestionTab> tabs,
      String fromLang, String toLang, GoogleTranslator translator) async {
    ...
    return Future.wait(tabs.map((tab) async {
      ...
      final titleFuture = _isTranslatableText(titleClean)
          ? _safeTranslate(titleClean, fromLang, toLang, translator)
          : Future<String>.value(tab.title);
      final textFuture = _isTranslatableText(textClean)
          ? _safeTranslate(textClean, fromLang, toLang, translator)
          : Future<String>.value('');
